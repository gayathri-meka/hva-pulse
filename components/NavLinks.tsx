...
type NavItem = {
  ...
  Icon:      () => JSX.Element
  children?: ChildLink[]
  // if true, "active" is also triggered by these extra prefixes
  extraPrefixes?: string[]
  // hide from guests (read-only role)
...
export default function NavLinks({ role }: { role: 'admin' | 'staff' | 'guest' }) {
  ...
  return (
    <nav className="flex flex-col gap-0.5 px-3">
      {links.map(({ href, label, Icon, children, extraPrefixes }) => {
        const allPrefixes = [href, ...(extraPrefixes ?? [])]
        const active = allPrefixes.some((p) => pathname === p || pathname.startsWith(p + '/'))
        ...
        return (
          <div key={href}>
            ...
            {children && active && (
              <div className="ml-4 mt-0.5 flex flex-col gap-0.5 border-l border-zinc-700 pl-3">
                {children.map((child) => {
