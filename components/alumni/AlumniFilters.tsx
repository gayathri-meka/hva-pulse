...
export default function AlumniFilters({ fyYears }: Props) {
  const router       = useRouter()
  const searchParams = useSearchParams()

  const activeFy     = searchParams.get('fy')     ?? ''
  const activeStatus = searchParams.get('status') ?? ''
  const isFiltered   = !!(activeFy || activeStatus)
  ...
  return (
    <div className="mb-6 flex flex-wrap items-center gap-2">
      <div className="relative">
        <select value={activeFy} onChange={(e) => update('fy', e.target.value)} className={selectCls}>
      ...
      <div className="relative">
        <select value={activeStatus} onChange={(e) => update('status', e.target.value)} className={selectCls}>
