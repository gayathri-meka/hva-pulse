...
describe('ApplicationsList', () => {
  ...
  test('changing to rejected opens the feedback modal', () => {
    ...
    expect(screen.getByText('Why was this candidate rejected?')).toBeInTheDocument()
  })
  ...
  test('modal confirm without selecting a reason shows validation error', () => {
    render(<ApplicationsList applications={[makeApp()]} {...defaultProps} />)
    fireEvent.change(screen.getByRole('combobox'), { target: { value: 'not_shortlisted' } })
  ...
  test('modal confirm with reason (and optional note) calls updateApplicationStatus and closes modal', async () => {
    render(<ApplicationsList applications={[makeApp()]} {...defaultProps} />)
    fireEvent.change(screen.getByRole('combobox'), { target: { value: 'not_shortlisted' } })
    fireEvent.click(screen.getByLabelText('Skill Mismatch'))
    ...
    await waitFor(() => {
      expect(updateApplicationStatus).toHaveBeenCalledWith(
        'app-1', 'not_shortlisted', 'Stronger candidates applied', ['Skill Mismatch'],
      )
    })
    expect(screen.queryByText("Why wasn't this candidate shortlisted?")).not.toBeInTheDocument()
  })

  test('modal confirm with only a reason and no note calls updateApplicationStatus', async () => {
    render(<ApplicationsList applications={[makeApp()]} {...defaultProps} />)
    fireEvent.change(screen.getByRole('combobox'), { target: { value: 'not_shortlisted' } })
    fireEvent.click(screen.getByLabelText('Skill Mismatch'))
    fireEvent.click(screen.getByRole('button', { name: 'Confirm' }))
    ...
    await waitFor(() => {
      expect(updateApplicationStatus).toHaveBeenCalledWith(
        'app-1', 'not_shortlisted', undefined, ['Skill Mismatch'],
      )
    })
  })
