...
OPENAI_API_KEY                  # ask-pulse-evals suite + AI interview-notes review (lib/openai.ts)
...
1. **System decision** — a pure, deterministic rule engine (`lib/challengeReview.ts`, `evaluateCandidate`) scores each candidate's signals against editable thresholds and yields **`in_progress` / `selected` / `rejected` / `review`** (four states) plus per-criterion pass/fail. Recomputed live on every render from `metric_raw_rows` (never persisted for its own sake). **Precedence:** still mid-challenge (**`challengeFinished === false`**) → **`in_progress`** — NO rules run (the 14-day-window rules like span/gap can't be judged yet, so no mid-challenge learner is ever rejected/messaged); else any hard **fail** → `rejected`; else any **`undetermined`** gate (na *because the candidate's own data is missing on an ACTIVE gate*) → **`review`** (don't auto-select — a human resolves the blank); else all pass → `selected`. **"Finished"/evaluable** (`isChallengeFinished`, computed in the page): a candidate becomes evaluable once they've **attempted every quiz question** (completed), OR gone **inactive for ≥ `INACTIVITY_DAYS` (7) days** (measured from their last activity), OR the editable cohort **`challenge_end_date`** has passed (migration 071 — the backstop for never-started learners, who have no last-activity to measure from, so they stay pending until then). Everyone else — still actively working through the challenge, within 7 days, not yet done — stays `in_progress`. `in_progress` is NOT confirmable/releasable (bulk-confirm skips it, like `review`). A **not-yet-configured** gate is `na` but NOT `undetermined` (stays neutral) — so unconfigured thresholds don't flag everyone. `review` is NOT a confirmable verdict: `bulkConfirmChallengeDecisions` skips review rows (they need a manual decision in the drawer); `final_decision` stays `selected|rejected` only. Criteria carry a `group`:
...
**Candidate side — Challenge tab (`/candidate/challenge`):** five states driven by the candidate's own synced progress (`candidateChallengeProgress` over their Challenge-Completion rows, by email via service-role) + `challenge_decisions`: **not started** (Start CTA) · **in progress** (bold %-bar "You're X% there, <name>!" + Log-in-to-SensAI CTA; avoids the word "attempt") · **completed** (100% items → celebration, "wait for HVA update") · **selected** (released → "through to the interview" + link to the Interview tab) · **rejected** (released → placeholder; templated reject messages TBD). "How it works" + "What we observe" persist under every state. The layout marks the Challenge stepper pill green once a decision is released.

**Candidate side — Selection tab:** `/candidate/selection` renders three states from `final_decision` (pending "under review" / selected "through to interview, wait" / rejected with **codified feedback** = the failed criteria's `failFeedback` strings from the snapshot). Portal-only, no email. The `/candidate/welcome` stepper advances the "Current" pill on decision (selected → Interview, rejected → Selection). The interview flow itself is a separate build.
...
- **Interviewers are specialised by round** (migration 081): `users.interview_round` (1=Motivation, 2=Coding) — each interviewer runs ONE panel, set/edited in the Overview tab (the "Panel" column). Their published availability inherits their round (`interview_slots.round`, stamped in `syncWeekAvailability`), and candidate booking only offers slots whose `round` matches the round they're on (`getBookingState`/`bookSlot` filter + atomic claim on `round`). So a Coding-round candidate can never book a Motivation interviewer's slot. Coding is a **separate panel** from Motivation.
...
- **Availability = "paint your week" grid** (`calendar/AvailabilityCalendar.tsx`): a 24×7, 1-hour-cell week calendar, drag to add/remove availability, **fixed 1-hour slots** (one cell = one bookable slot; `SLOT_MINUTES = 60`), "copy week → next", rolling 2 weeks, IST; booked interviews render locked (blue) on the grid + a list below with Done/No-show. `syncWeekAvailability(weekStart, starts[])` reconciles that week's OPEN slots to the painted set (creates missing, deletes removed-open, never touches booked/past). Candidate booking (`app/candidate/interview/`) unchanged — gated on `selected`+`published_at`, books the next round, round-2 pool excludes the round-1 interviewer, atomic `open→booked` claim then insert (rolls slot back on failure).
...
- **Booking → Calendar invite + Meet link (BUILT):** on booking, `bookSlot` creates a Google Calendar event via `lib/googleCalendar.ts` (`createInterviewEvent`) using the shared Google account (Settings → Email) — that account (academy@…) is the **organiser**; interviewer + candidate are **attendees**; `conferenceData`/`conferenceDataVersion=1` mints the **Meet link**; `sendUpdates=all` makes Google email the invite to everyone. `meet_link` + `calendar_event_id` are stored on the interview; cancel deletes the event. Falls back to the plain `lib/interviewInvite.ts` email only when Calendar isn't connected. **Requires:** `calendar.events` in `GMAIL_SCOPES` (done), Calendar API enabled, and the shared account connected (the connected account is the organiser — connect academy@hyperverge.co). **Phase B** (interview cockpit: dossier + question bank + notes + rubric scoring) is BUILT (`/admissions/interviews/notes/[id]`). **Phase C** (AI second-opinion + live weak-answer flag) is not built. See the `interview-flow-design` memory for the full plan.

**Interview Review tab + release gates (BUILT, migration 082):** the last Interviews sub-tab (`/admissions/interviews/review`) is where the team drives the pipeline for challenge-selected+released candidates. Two release gates in `interview_decisions` (per candidate): **stage1** (`advance`|`rejected`) after Round 1, **final** (`selected`|`rejected`) after Round 2. Recording a value **is** the release (no separate publish step). Releasing **advance** is what unlocks Round-2 (Coding) booking — `getBookingState`/`bookSlot` gate Round 2 on `stage1==='advance'`, and any rejection/final call closes booking. The interviewer's per-round `recommendation` (advance/borderline/no) is **guidance only** — the team can release either way. Pure state machine in `lib/interviewReview.ts` (`computeInterviewReviewRow`, unit-tested); actions in `review-actions.ts` (`getInterviewReview`, `setInterviewDecision`, `undoInterviewDecision`, `requireStaff`). Candidate side: the Interview tab shows a rejection / "under review" / "you're through" banner off `stage1`/`final`; the **Selection** step (5) unlocks only on `final==='selected'` (rejection leaves it greyed), and `/candidate/selection` shows the selected celebration. Rejection copy is **generic for now — codify per-reason messages later** (like the challenge flow). "Select" is **record-only** (no onboarding/learner creation yet). Migration 082 must be applied; schema-dump regen owed.

**Rubrics — flexible scales + round-scoped (migration 083):** a rubric's scale is a variable `levels` list (`interview_rubrics.levels` jsonb, each `{score, descriptor, looking_for, example}`), so scales can be **fractional/uneven** (Articulation = 1, 2, 2.5, 3, 3.5, 4) and needn't start at 1 (Reading Comprehension = 2, 3, 3.5, 4). `interview_scores.score` is **numeric**. Rubrics carry a `round` (1|2|null) and are filtered per-round in the cockpit (`rubricsForRound`), like questions. `toRubric` normalises jsonb `levels`, falling back to the legacy `level_1..4` columns for rubrics not yet re-saved. Pure helpers in `lib/interviewCockpit.ts` (`rubricsForRound`, `isValidScoreForRubric`, `scoreTone`, `formatScore`, `averageScore`); `saveScore` validates the score against the rubric's own scale. Score chips are coloured by `scoreTone(value)` (band, not a 1–4 index) and render fractional values (`formatScore`). The Questions & rubrics editor edits variable levels (add/remove, per-level score) + round + a per-rubric interviewer note. Motivation rubrics: Need, Drive, Program Alignment, Time Commitment, **Articulation** (6-pt), **Listening Comprehension** (5-pt, was "Comprehension"); Coding: **Reading Comprehension** (4-pt). Migration 083 must be applied.

**AI notes review (built, per-interview):** a subtle "✨ AI check" button on each row of the Interviews → **Notes** tab. It calls `reviewInterviewNotes(interviewId)` (`cockpit-actions.ts`, `requireStaff`), which gathers that interview's per-question notes + the rubric level descriptors + the interviewer's 1–4 scores + summary, and asks OpenAI (via `lib/openai.ts` `chatJSON`, model `gpt-4o`, JSON mode) to: (1) rate overall note quality (strong/adequate/thin), (2) flag **specific questions** whose notes are missing/thin, and (3) for each rubric, judge whether the notes **justify the score given** or suggest a different score grounded in the rubric levels. Prompt-building + response normalisation are pure in `lib/interviewNotesReview.ts` (unit-tested); the result renders in a modal (`components/interviews/NotesReviewButton.tsx`). It's a second opinion, not a decision. **Next planned:** a per-answer version inside the cockpit (live feedback on a single note as it's written).
...
### Tech Support Tickets (shared with the hva-automation Slack bot)

Tech Support tickets are raised via the **`/tech-support` Slack flow** handled by the `hva-slack-router` **edge function in the separate `hva-automation` repo** (`supabase/functions/hva-slack-router/`). That edge function and Pulse **share ONE Supabase project** (`onukununwlcwowtgqzxl`), so the edge (service-role) writes tickets straight into Postgres and Pulse reads/acts on them. **Postgres is the single source of truth**; the old Google Sheet + Apps Script store is **retired** (`tech-support/Code.js` in hva-automation is deprecated).

**Tables (migration `088_tickets.sql`), all RLS `staff_all` (admin/staff):**
- `tickets` — one row per ticket, natural key `ticket_id` (the Slack view id). Holds content, requester, `spocs` (Slack ids, for @-tagging) + `spoc_emails` (the identity bridge for Pulse gating), `status` (open/closed/escalated), Slack refs (`channel_id/ts`, `dm_channel_id/ts` — needed so Pulse/cron can repaint cards + thread replies), `is_test`, reminder counters, `closed_*`, and **`rating` (1/2/3 CSAT) + `feedback`** (requester close-feedback).
- `ticket_events` — audit log (close/reopen/edit/reply/rating/feedback/reminder). **Reply BODIES are NOT stored** — the conversation lives in the Slack thread (the edge mirrors every reply into the channel thread); only that a reply happened is logged.
- `ticket_categories` — dynamic category list + SPOC assignment. **SPOCs are the app's admin/staff USERS** (`public.users`), referenced by **email** in `spoc_emails`; the Slack id for @-tagging is resolved at runtime from the email via Slack `users.lookupByEmail`. Edited from Pulse (**Manage categories**), read by the edge when building the Slack modal — no redeploy. (Migration `089` = pg_cron for the daily reminders + weekly summary.)

**Architecture — the edge is the single mutation point.** Slack clicks, Pulse actions, and the crons all flow through the edge, which does the Slack work AND writes Postgres. **Pulse never calls Slack directly** — its reply/close/reopen/edit actions POST to the edge (`source:'pulse'` + shared secret `HVA_PULSE_EDGE_SECRET`, the only new Pulse env var; the edge URL is derived from `NEXT_PUBLIC_SUPABASE_URL`). The edge rebuilds the Slack cards from the Postgres row.

**Pulse module** `app/(protected)/tickets/`: `page.tsx` (`requireStaff`, list), `[ticketId]/page.tsx` (detail + audit log + full feedback), `TicketsTable.tsx` (`DataTable`, actions pinned right), `settings/` (Manage categories, admin-only), `actions.ts` (POST to edge). **Permission gating** (`lib/tickets.ts` `canActOnTicket`, unit-tested): **admin OR the ticket's assigned SPOC by email** — everyone else is view-only. `lib/tickets.ts` also holds the status/priority/CSAT display helpers. Status colours: **Open = red, Escalated = solid red, Closed = green**.

**Feedback (CSAT):** after close, the requester's DM gets (7s later) a 😞/😐/😄 prompt + a "Tell us more" comment modal; stored as `rating`/`feedback`, shown in the CSAT column (💬 flags a comment) + detail page. **Reminders:** daily Mon–Fri 10 AM IST cron → edge, cadence High 1d/Med 2d/Low 3d from `last_reminded_at`, escalate after 3. **Weekly summary:** Monday 9 AM IST cron → per-SPOC Block Kit report to the main channel. Crons are `pg_cron → net.http_post` to the edge (`source:'cron'`, `kind` = `run_reminders` | `run_weekly_summary`); secrets in Supabase Vault. Migration `089` + Vault setup are **manual** (needs a project Owner). Middleware + `ADMIN_LINKS`/`STAFF_LINKS` include `/tickets`.
...
- **Data tables: use the shared `components/ui/DataTable.tsx` for EVERY new table — this is the standard, do not hand-roll a new TanStack table.** Pass `data`, `columns` (TanStack `ColumnDef[]`), and a `storageKey`; it provides sticky header, sorting, per-column filters (search-within + faceted counts), a column show/hide menu, persisted sizing + visibility, CSV export, search box, row count, "Clear filters", and `toolbarLeft/right` slots. Key props: `pinnedLeft` / `pinnedRight` (pinned on desktop, **auto-unpinned on mobile** so frozen columns don't break narrow screens — use `pinnedRight={['actions']}` to keep an Actions column visible when the table scrolls horizontally), `searchKeys`, `csvFilename`, `getRowId`, `rowClassName`, `enableRowSelection` (adds a checkbox column). The table **fills its container width by default** (no trailing gap) and scrolls when columns are wider than the viewport. Truncated string cells show a **styled `Tooltip`** with the full value on hover (only when actually clipped) — no native `title` needed. `toolbarLeft`/`toolbarRight` accept either a node or a render function `({ selectedRows, filteredRows, clearSelection }) => node` — that's how a toolbar action (e.g. the email button) targets the picked rows, falling back to the filtered rows when nothing is selected. A column opts out of filtering with `enableColumnFilter: false` (do this for date columns) and out of hiding with `enableHiding: false` (do this for action/link columns); array-valued columns supply their filter options via `meta.facetOptions` and a custom `filterFn`. **Make every data column a `col.accessor`** returning a filterable/sortable string (render richer UI via its `cell`) so its per-column filter works — reserve `col.display` for pure action/link columns. A long header wraps to two lines via `meta.wrapHeader` (reduced padding + centered) instead of truncating. Reference implementations: `app/(protected)/admissions/{learner-applications,prospects}` tables and `app/(protected)/admissions/interviews/review/ReviewTable.tsx` (per-round Status/Decision/Notes columns, all accessor-based + filterable, release actions behind a confirm modal). Pure filter helpers live in `lib/tableFilters.ts`.
...
- **Tooltips: use the shared `components/ui/Tooltip.tsx`** for hover hints instead of the browser's native `title`. It renders a styled dark bubble via a **portal** (so it's never clipped by a scroll/`overflow-hidden` ancestor) and only mounts the tooltip DOM while hovered (cheap enough for every table cell). Usage: `<Tooltip content="Full text"><button>…</button></Tooltip>`. Pass `truncate` to make the trigger a single-line truncating block that only shows the tooltip when the text actually overflows (this is how `DataTable` cells use it). For long content (e.g. a feedback comment), the tooltip is a **preview** — surface the full text on a detail page, and flag its presence in the cell (e.g. a 💬 icon), rather than relying on hover alone.
