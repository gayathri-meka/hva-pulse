...
**Tables (migration `086_tickets.sql`), all RLS `staff_all` (admin/staff):**
...
- `ticket_categories` — dynamic category list + SPOC assignment. **SPOCs are the app's admin/staff USERS** (`public.users`), referenced by **email** in `spoc_emails`; the Slack id for @-tagging is resolved at runtime from the email via Slack `users.lookupByEmail`. Edited from Pulse (**Manage categories**), read by the edge when building the Slack modal — no redeploy. (Migration `087` = pg_cron for the daily reminders + weekly summary.)
...
**Feedback (CSAT):** after close, the requester's DM gets (7s later) a 😞/😐/😄 prompt + a "Tell us more" comment modal; stored as `rating`/`feedback`, shown in the CSAT column (💬 flags a comment) + detail page. **Reminders:** daily Mon–Fri 10 AM IST cron → edge, cadence High 1d/Med 2d/Low 3d from `last_reminded_at`, escalate after 3. **Weekly summary:** Monday 9 AM IST cron → per-SPOC Block Kit report to the main channel. Crons are `pg_cron → net.http_post` to the edge (`source:'cron'`, `kind` = `run_reminders` | `run_weekly_summary`); secrets in Supabase Vault. Migration `087` + Vault setup are **manual** (needs a project Owner). Middleware + `ADMIN_LINKS`/`STAFF_LINKS` include `/tickets`.
