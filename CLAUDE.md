...
- **Interviewers are Pulse users with role `interviewer`** (migration 069 adds it to `users_role_check`) — for people who ONLY interview; they log in via Google and are redirected to the scoped **`/interviewer`** portal (`(protected)/layout.tsx`, like `learner`). **Admin + staff also get interviewer privileges automatically** (`requireInterviewer` allows admin/staff/interviewer) — they reach `/interviewer` via the "Open interviewer view" link on the admin Interviews tab (they aren't auto-redirected there; only the dedicated `interviewer` role is). The admin Interviewers list + name resolution include any admin/staff who publish slots or conduct interviews, not just role-`interviewer` users. Admin adds dedicated interviewers in Admissions → Interviews (or Users). `/interviewer` is in `middleware.ts` protectedPrefixes + matcher.
...
- **Surfaces:** interviewer portal `app/interviewer/` (publish availability, see schedule, mark completed/no-show); candidate `app/candidate/interview/` (gated on `selected`+`published_at`, book a slot for the next round, round-2 pool excludes the round-1 interviewer); admin `app/(protected)/admissions/interviews/` (add interviewers, slot pool, all bookings, metrics: open/booked/scheduled/completed/no-show/show-rate).
- Pure logic (`validateNewSlot`, `nextBookableRound`, `isSlotBookable`, `computeInterviewMetrics`) in `lib/interviews.ts` (unit-tested). Booking atomically claims a slot (`open→booked` only if still open) then inserts the interview; rolls the slot back on failure.
