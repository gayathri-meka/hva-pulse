...
- **Interviewers are Pulse users with role `interviewer`** (migration 069 adds it to `users_role_check`). They log in via Google and are redirected to a scoped **`/interviewer`** portal (`(protected)/layout.tsx`, like `learner`; `requireInterviewer` in `lib/auth`). Admin adds them in Admissions → Interviews (or Users). `/interviewer` is in `middleware.ts` protectedPrefixes + matcher.
