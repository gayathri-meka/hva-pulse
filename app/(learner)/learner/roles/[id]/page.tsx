...
export default async function RoleDetailPage({ params }: Props) {
  ...
  const hasJd      = !!role.jd_attachment_url
  const canApply   = role.status === 'open' && !application
  const showStickyBar = hasJd || canApply

  return (
    <>
      {/* Extra bottom padding so content isn't hidden behind the sticky bar */}
      <div className={`mx-auto max-w-2xl px-4 py-6 ${showStickyBar ? 'pb-32 md:pb-24' : ''}`}>
        {/* Back */}
        <Link
          href="/learner"
      ...
      {/* Sticky bottom bar */}
      {showStickyBar && (
        <div className="fixed bottom-16 left-0 right-0 z-40 border-t border-zinc-200 bg-white/95 px-4 py-3 backdrop-blur-sm md:bottom-0">
          <div className="mx-auto flex max-w-2xl gap-3">
            {hasJd && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-zinc-200 px-4 py-3 text-sm font-semibold text-zinc-700 transition-colors hover:bg-zinc-50"
              >
            ...
            {canApply && (
              <a
                href="#apply"
                className="flex flex-1 items-center justify-center rounded-xl bg-zinc-900 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-zinc-700"
              >
                Apply →
              </a>
            )}
