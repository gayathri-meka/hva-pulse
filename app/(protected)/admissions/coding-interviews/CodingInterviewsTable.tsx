...
export default function CodingInterviewsTable({ rows: initialRows }: { rows: CodingInterviewRow[] }) {
  ...
  const isPending = (email: string, field: CodingInterviewField) => pendingCells.has(`${email}:${field}`)
  ...
  const columns = useMemo(() => [
    ...
    col.accessor((r) => r.interviewStatus === 'not_started' ? 'Not Started' : 'Completed', {
      ...
      cell: (i) => <SelectCell value={i.row.original.interviewStatus} options={[['not_started', 'Not Started'], ['completed', 'Completed']]} disabled={isPending(i.row.original.email, 'interviewStatus')} onChange={(v) => save(i.row.original.email, 'interviewStatus', v)} />,
    }),
    col.accessor((r) => r.verdict ?? '', {
      id: 'verdict', header: 'Verdict', size: 130,
      cell: (i) => <SelectCell value={i.row.original.verdict ?? ''} placeholder="—" options={[['selected', 'Selected'], ['rejected', 'Rejected']]} disabled={isPending(i.row.original.email, 'verdict')} onChange={(v) => save(i.row.original.email, 'verdict', v || null)} />,
    }),
    col.accessor((r) => r.interviewDate ?? '', { id: 'date', header: 'Date', size: 145, enableColumnFilter: false, cell: (i) => <InputCell type="date" value={i.row.original.interviewDate ?? ''} disabled={isPending(i.row.original.email, 'interviewDate')} onSave={(v) => save(i.row.original.email, 'interviewDate', v || null)} /> }),
    col.accessor((r) => r.interviewTime ?? '', { id: 'time', header: 'Time', size: 125, enableColumnFilter: false, cell: (i) => <InputCell type="time" value={i.row.original.interviewTime ?? ''} disabled={isPending(i.row.original.email, 'interviewTime')} onSave={(v) => save(i.row.original.email, 'interviewTime', v || null)} /> }),
    col.accessor('preInterviewNotes', { header: 'Pre-Interview Notes', size: 260, meta: { wrapHeader: true }, cell: (i) => <TextCell value={i.getValue()} disabled={isPending(i.row.original.email, 'preInterviewNotes')} placeholder="Add pre-interview notes…" onSave={(v) => save(i.row.original.email, 'preInterviewNotes', v)} /> }),
    col.accessor((r) => r.interviewer ?? '', { id: 'interviewer', header: 'Interviewer', size: 140, cell: (i) => <SelectCell value={i.row.original.interviewer ?? ''} placeholder="—" options={CODING_INTERVIEWERS.map((v) => [v, v])} disabled={isPending(i.row.original.email, 'interviewer')} onChange={(v) => save(i.row.original.email, 'interviewer', v || null)} /> }),
    col.accessor('problemsAsked', { header: 'Problems Asked', size: 240, meta: { wrapHeader: true }, cell: (i) => <TextCell value={i.getValue()} disabled={isPending(i.row.original.email, 'problemsAsked')} placeholder="Add problems asked…" onSave={(v) => save(i.row.original.email, 'problemsAsked', v)} /> }),
    col.accessor((r) => r.codingScore == null ? '' : String(r.codingScore), { id: 'codingScore', header: 'Coding Score', size: 135, meta: { wrapHeader: true }, cell: (i) => <ScoreCell value={i.row.original.codingScore} disabled={isPending(i.row.original.email, 'codingScore')} onChange={(v) => save(i.row.original.email, 'codingScore', v)} /> }),
    col.accessor((r) => r.readingComprehensionScore == null ? '' : String(r.readingComprehensionScore), { id: 'readingComprehensionScore', header: 'Reading Comprehension Score', size: 190, meta: { wrapHeader: true }, cell: (i) => <ScoreCell value={i.row.original.readingComprehensionScore} disabled={isPending(i.row.original.email, 'readingComprehensionScore')} onChange={(v) => save(i.row.original.email, 'readingComprehensionScore', v)} /> }),
    col.accessor('learnabilityObservations', { header: 'Learnability Observations', size: 260, meta: { wrapHeader: true }, cell: (i) => <TextCell value={i.getValue()} disabled={isPending(i.row.original.email, 'learnabilityObservations')} placeholder="Add observations…" onSave={(v) => save(i.row.original.email, 'learnabilityObservations', v)} /> }),
    col.accessor('notes', { header: 'Notes', size: 240, cell: (i) => <TextCell value={i.getValue()} disabled={isPending(i.row.original.email, 'notes')} placeholder="Add notes…" onSave={(v) => save(i.row.original.email, 'notes', v)} /> }),
    col.accessor('summary', { header: 'Summary', size: 260, cell: (i) => <TextCell value={i.getValue()} disabled={isPending(i.row.original.email, 'summary')} placeholder="Add summary…" onSave={(v) => save(i.row.original.email, 'summary', v)} /> }),
  ], [pendingCells])
