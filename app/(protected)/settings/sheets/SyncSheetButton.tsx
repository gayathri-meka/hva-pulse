...
export default function SyncSheetButton({ apiRoute, label = 'Sync now' }: Props) {
  ...
  async function handleSync() {
    ...
    try {
      ...
      if (data.success) {
        setMessage(`${data.count} records synced`)
        router.refresh()
      } else {
        setIsError(true)
  ...
  return (
    <div className="flex items-center gap-3">
      {message && (
        <span className={`text-xs font-medium ${isError ? 'text-red-500' : 'text-emerald-600'}`}>
          {message}
        </span>
      )}
      <button
